...
std::string IpftSearchStatistics::text() const {
  ...
  ss << secSep << "Actions(" << to_string(this->valuedActions.size()) << ")"
     << endl;
  ...
  ss << this->printTimes();
  ss << secSep << "Nodes/Visits on level (" << to_string(this->num_tree_vnodes)
     << "nodes, " << to_string(this->num_simulations) << "sims)" << endl;
  ss << printNodeStats();
  ss << secSep << "Deepest sim depth(" << this->deepest_simulation_depth << ")"
     << endl;
  ss << separator << endl;
  return ss.str();
}
...
std::string IpftSearchStatistics::printValuedActions() const {
  ...
  for (int i = 0; i < this->valuedActions.size(); i++) {
    ...
    ss << left << setw(15) << setfill(fillChar)
       << this->model_->to_string(this->valuedActions[i].action_)
...
std::string IpftSearchStatistics::printTimes() const {
  ...
  constexpr char fillChar = ' ';
  stringstream ss;
  ss << left << setw(width1) << setfill(fillChar) << "node selection:" << right
     << setw(width2) << setfill(fillChar) << to_string(time_node_selection)
     << "ms" << endl;
  ss << left << setw(width1) << setfill(fillChar) << "belief update:" << right
     << setw(width2) << setfill(fillChar) << to_string(time_belief_update)
     << "ms" << endl;
  ss << left << setw(width1) << setfill(fillChar) << "inf gain calc:" << right
     << setw(width2) << setfill(fillChar)
     << to_string(time_information_gain_computation) << "ms" << endl;
  ss << left << setw(width1) << setfill(fillChar) << "node rollout:" << right
     << setw(width2) << setfill(fillChar) << to_string(time_node_rollout)
     << "ms" << endl;
  ss << left << setw(width1) << setfill(fillChar) << "backup:" << right
     << setw(width2) << setfill(fillChar) << to_string(time_backup) << "ms"
     << endl;
...
std::string IpftSearchStatistics::printNodeStats() const {
  ...
  for (int i = 0; i < this->num_visits_vnodes_on_level.size(); i++) {
    ...
    ss << right << setw(width2) << setfill(fillChar)
       << to_string(num_vnodes_on_level[i]) << " | ";
    ss << right << setw(width2) << setfill(fillChar)
       << to_string(num_visits_vnodes_on_level[i]) << endl;
  }
  return ss.str();
}
...
int IpftValue::getComponentCount() const { return componentCount; }
...
ValuedAction Ipft::search() { return search(Globals::config.time_per_move); }
...
ValuedAction Ipft::search(double timeout) {
  ...
  do {
    ...
    if (Globals::config.convergence_eval) {
      ...
      this->stats_->valuedActionsPerTimestep.push_back(
  ...
  if (Globals::config.record_statistics) {
    ...
    for (const ValuedAction &valAct : this->stats_->valuedActions) {
      ...
      actObsSeq =
          this->root_->maximumValueActionObservationSequence(valAct.action_);
...
IpftValue Ipft::simulate(const std::shared_ptr<VNode> &vnode, int depth) {
  ...
  DLOG(INFO) << "[SIM] select " << model_->to_string(actionNode->getAction())
             << ", N(ha)=" << actionNode->getCount();

  //* Progressive widening for Observation space
  double pw_limit =
      Globals::config.pw_k_obs *
      std::pow(actionNode->getCount(), Globals::config.pw_alpha_obs);
  ...
  DLOG(INFO) << "[SIM] generate " << model_->to_string(o) << " from "
             << model_->to_string(s)
             << model_->to_string(actionNode->getAction())
             << model_->to_string(statePosterior);
  ...
  if (actionNode->children().size() <= pw_limit) {
    ...
    int randomIndex =
        this->rand_->nextUniformInt(actionNode->children().size());
    ...
    DLOG(INFO) << "[SIM] select " << model_->to_string((obsNode->obsEdge_))
  ...
  auto startBelUpdate =
      std::chrono::high_resolution_clock::now(); // time_belief_update
  Action act = actionNode->getAction();
  double stateReward = bNext->update(act, *obs);
  CHECK(!std::isnan(stateReward)) << "State reward is nan.";
  this->stats_->time_belief_update +=
      stopTime(startBelUpdate); // time_belief_update
...
ValuedAction Ipft::optimalAction(const VNode &vnode) const {
  ...
  CHECK_NE(static_cast<int>(astar.action_), -1)
...
std::shared_ptr<QNode> Ipft::ucbActionSelection(const VNode &vnode) const {
  ...
  for (auto &qnode : qnodes) {
    ...
    DLOG(INFO) << "[UCB] " << model_->to_string(qnode->getAction())
