...
void Plannerbase::displayParameters() const {
  ...
  std::cout
      ...
      << "Search time per step = " << Globals::config.time_per_move << "ms"
      << endl
      << "Max. simulations count per search = "
      << Globals::config.max_simulation_count << endl
      << "Number of simulation steps = " << Globals::config.sim_len << endl
      << "Number of solver particles = " << Globals::config.num_solver_particles
      << endl
      << "Number of search particles = " << Globals::config.num_search_particles
      << endl
      << "Progressive widening k_obs = " << Globals::config.pw_k_obs << endl
      << "Progressive widening alpha_obs = " << Globals::config.pw_alpha_obs
      << endl
      ...
      << "Information gathering const. lambda = "
      << Globals::config.inf_gather_constant_lambda << endl
...
void Plannerbase::printPlanningEnd(
  ...
  std::chrono::duration<double, std::milli> elapsed = end - main_clock_start;
