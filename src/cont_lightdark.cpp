...
CLDActionValue::CLDActionValue(const Action& act) : CLDObs(static_cast<double>(actToValueMap.at(act))) {
}
...
std::unique_ptr<Belief> ContLightDark::initialBelief(const std::string& type) {
    ...
    return std::make_unique<ParticleBelief>(
        ...
        shared_from_this(),
        this->rand_,
        std::make_unique<ObsAdaptiveReinvigorator>(shared_from_this(), this->rand_));
}
...
bool ContLightDark::terminalState(const Action& action, const State& statePosterior) const {
    auto cldAction = static_cast<CLDAction>(action);
...
Observation* ContLightDark::copyObs(const Observation* obs) const {
    ...
    *newObs = *dynamic_cast<const CLDObs*>(obs); // default copy constructor called
    newObs->SetAllocated();
    return newObs;
}
...
State* ContLightDark::copyState(const State* state) const {
    ...
    *newState = *dynamic_cast<const CLDState*>(state); // default copy constructor called
