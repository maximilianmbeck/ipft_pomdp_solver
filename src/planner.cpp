...
int Planner::runPlanning(int argc, char* argv[]) {
    ...
    // TODO option parser --> gflags library
...
void Planner::initializePlanner() {
    //* initialize model
    POMDP* model = initializeModel();  // the model is shared between the world and the solver
    this->model_ = model;
    //* initialize world
    this->world_ = initializePOMDPWorld(model);  // TODO: concept for use of different model
...
void Planner::resetPlanner() {
    ...
    if (this->sim_stats_ != nullptr) delete this->sim_stats_;
