...
void Planner::runPlanningLoop() {
    ...
    this->sim_stats_->endRound(this->step_, std::move(solverHist), std::move(worldHist));
}
...
void Planner::initializePlanner() {
    ...
    this->sim_stats_ = std::make_unique<SimulationStatistics>(this->model_, this->world_, this->solver_, &std::cout);
