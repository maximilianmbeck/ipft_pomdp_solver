...
double
KernelDensityEstimator::computeDensityValue(const State *s,
                                            const ParticleBelief *b) const {
  int dimensions = b->model_->numDimStateSpace();
  double densityEstimate = 0.0;
  if (dimensions <= 1) {
    // univariate case
    double bandwidth = bwSelector_->computeBandwidth(
        b); // TODO(max): performance improvement! do this only once or
    ...
    std::vector<double> bwMatrixDiagElmts =
        bwSelector_->computeBandwidthMatrixDiagElmts(
            b); // TODO(max): same as above
...
double RotBandwidthSelector::computeBandwidth(const ParticleBelief *b) const {
  ...
  return std::max(1.0592238410488122 *
                      std::pow(static_cast<double>(b->numParticles()), -0.2) *
                      stdeviation,
                  minBandw);
}

std::vector<double> RotBandwidthSelector::computeBandwidthMatrixDiagElmts(
    const ParticleBelief *b) const {
  int dimensions = b->model_->numDimStateSpace();
  State *wSampleStd = b->std();
  ...
  for (int dim = 0; dim < dimensions; dim++) {
    ...
    bandwidthMatrixDiagElmts[dim] *= std::pow(
        4.0 / (b->numParticles() * (dimensions + 2)), 1.0 / (dimensions + 4));
  }
  b->model_->freeState(wSampleStd);
...
double NormalKernel::univariateValue(const State *s,
...
double
NormalKernel::multivariateValue(const State *s, const ParticleBelief *belief,
