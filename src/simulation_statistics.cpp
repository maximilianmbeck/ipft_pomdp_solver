...
void SimulationStatistics::endRound(int step_count, History &&solver_hist,
  ...
  std::chrono::duration<double, std::milli> elapsed =
      round_clock_end - this->round_clock_start;
  ...
  *out_ << "Total discounted reward = " << this->total_discounted_reward_
        << std::endl;
  // print total undiscounted reward
  *out_ << "Total undiscounted reward = " << this->total_undiscounted_reward_
        << std::endl;
}

void SimulationStatistics::summarizeRewards(int round, int step,
                                            double reward) {
  // update rewards
  ...
  *out_ << "- Current rewards:" << std::endl
        ...
        << total_undiscounted_reward_ << std::endl;
}
...
void SimulationStatistics::printRoundResultsTable(std::ostream &os) const {
  ...
  if (this->step_count_ > 0) {
    ...
    // ObsProb | Solv.MeanPost | Solv.StdPost | Reward
    // ------------------------------------------------------------------------------------------------------------------------

    vector<string> cols = {"Step",         "   State", "Solv.Mean",
                           "Solv.Std",     "  Action", "StatePost",
                           "Observation",  "ObsProb",  "Solv.MeanPost",
                           "Solv.StdPost", "Reward"};
    ...
    // adapt width to data
    CHECK_EQ(cols.size(), dataRow.size())
        << "Number of header colums does not match nuber of data colums.";
...
SimulationStatistics::getStepResultsForTable(int step) const {
