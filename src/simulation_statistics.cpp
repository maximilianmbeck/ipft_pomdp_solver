...
SimulationStatistics::SimulationStatistics(std::shared_ptr<POMDP> model,
                                           ...
                                           std::ostream* out)
        : model_(std::move(model)), world_(std::move(world)), solver_(std::move(solver)), out_(out),
...
void SimulationStatistics::endRound(int step_count, History&& solver_hist, std::vector<State*>&& world_hist) {
    ...
    std::chrono::duration<double, std::milli> elapsed = round_clock_end - this->round_clock_start;
...
void SimulationStatistics::summarizeRewards(int round, int step, double reward) {
    // update rewards
