...
Random::Random(int seedOffset) {
  ...
  _randEngine = std::make_unique<std::mt19937_64>(
...
std::mt19937_64 *Random::engine() const { return this->_randEngine.get(); }
...
unsigned Random::nextUnsigned() const { return (*_randEngine)(); }
...
int Random::nextDiscrProbDistrInt(
    const std::vector<double> &discreteProbDistrWeights) const {
  ...
  // int index = 0;
  // double accProb = 0.0;
  // std::generate(accProbDistr.begin(), accProbDistr.end(),
  // [&discreteProbDistr, &index, &accProb]() -> double {
  ...
  // use c++ stdlib
  std::discrete_distribution<> dd(discreteProbDistrWeights.begin(),
                                  discreteProbDistrWeights.end());
