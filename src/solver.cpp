...
Solver::Solver(std::shared_ptr<POMDP> model)
        : model_(std::move(model)), belief_(nullptr), root_(nullptr), history_(std::make_unique<History>(model_)) {
}
...
std::shared_ptr<VNode> Solver::getSearchTree() const {
    return this->root_;
}
...
const Belief* Solver::getBelief() const {
    return this->belief_.get();
}
