...
POMDP::copyObss(const std::vector<Observation *> &obss) const {
...
std::vector<State *>
POMDP::copyStates(const std::vector<State *> &states) const {
...
double POMDP::obsProb(const State &statePosterior,
                      const Observation &obs) const {
  throw std::runtime_error("POMDP::obsProb not supported!");
  return 0.0;
}

double POMDP::obsProb(const State &state, const Action &action,
...
double POMDP::reward(const State &state, const Action &action,
...
double POMDP::maxPossibleWeight(const Action &act,
                                const Observation &obs) const {
