...
ParticleSetToString::ParticleSetToString(std::shared_ptr<POMDP> model) : model_(std::move(model)) {
}
...
std::string ParticleSetToString::shortDescription(const std::vector<State*>& particleSet, bool terminated) const {
    ...
    ss << "[PBelief(" << particleSet.size() << "): Mean(" << *meanState << ") Std(" << *stdState << ") Terminal("
       << terminated << ")]";
...
std::string ParticleSetToString::particleTable(const std::vector<State*>& particleSet) const {
    ...
    for (int i = 0; i < particleSet.size(); i++) {
        ...
        ss << left << setw(smallColumnSize) << setfill(fillChar) << to_string(particleSet[i]->weight) << "| ";
