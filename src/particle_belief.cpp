...
void ParticleBelief::init() {
  if (std::abs(State::weightSum(particles_) - 1.0) > Globals::DOUBLEEQLIM) {
    LOG(FATAL) << "[ParticleBelief::ParticleBelief] Particle weights sum to "
               << State::weightSum(particles_) << " instead of 1." << std::endl;
  }
...
int ParticleBelief::numParticles() const {
  ...
  CHECK(this->num_particles_ == particle_size)
      << "Numbers of particles do not match.";
  return this->num_particles_;
}

std::string
ParticleBelief::text(const std::vector<State *> &particleSet) const {
  ParticleSetToString pbPrinter(this->model_);
  std::string description =
      pbPrinter.shortDescription(particleSet, this->beliefTerminated_);
  std::stringstream ss;
  ss << description;
  return ss.str();
}
...
std::string
ParticleBelief::detailedText(const std::vector<State *> &particleSet) const {
  ParticleSetToString pbPrinter(this->model_);
  std::string description =
      pbPrinter.shortDescription(particleSet, this->beliefTerminated_);
...
double ParticleBelief::update(const Action &action, const Observation &obs) {
  ...
  // log belief update
  DLOG(INFO) << "[PF] " << std::setfill(' ') << std::setw(7) << std::left
             << "update " << this->text() << " with "
  ...
  double total_new_weight =
      0; // for error message (corresponds to weight sum in julia code)
  ...
  bool reinvigorateParticles =
      this->afterResampleReinvigorator_->particleReinvigorationNeeded(
          predictedParticles, action, obs);
  ...
  std::vector<State *> resampledPart =
      this->sample(num_particles_, this->weighted_posterior_particles_);
  this->model_->freeStates(this->particles_);
  ...
  DLOG_IF(ERROR, debug::allParticlesEqual(this))
      << "[PF] After resampling: All particles equal to "
      << *(resampledPart[0]);
  ...
  if (reinvigorateParticles) {
    this->particles_ = this->afterResampleReinvigorator_->reinvigorate(
        this->particles_, action, obs);
    DLOG(INFO) << "[PF] " << std::setfill(' ') << std::setw(7) << std::left
               << "new " << this->text();
  }
...
void ParticleBelief::reinvigorateParticlesAfterResampling(
    const Action &act, const Observation &obs) {
...
std::vector<State *>
ParticleBelief::sample(int num, const std::vector<State *> &particleSet) const {
