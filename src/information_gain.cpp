...
double EntropyInfGain::computeEntropyEstimate(const Belief* b) const {
    ...
    for (int i = 0; i < pb->numParticles(); i++) {
        ...
        entropyValue += s->weight * temp2;
    }
    CHECK(!std::isnan(entropyValue)) << "Entropy value is not a number!";
    return entropyValue;
}

double EntropyInfGain::computeDiscInfGain(double discFactor, const Belief* bnext, const Belief* b) const {
    return (discFactor * this->computeEntropyEstimate(bnext) - this->computeEntropyEstimate(b)); // no minus needed
}

/* ----------------------------- NoInfGain class ---------------------------- */

double NoInfGain::computeDiscInfGain(double discFactor, const Belief* bnext, const Belief* b) const {
    return 0.0;
}
} // namespace solver_ipft