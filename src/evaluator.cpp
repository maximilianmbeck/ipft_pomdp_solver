...
void EvaluationStatistics::summarizeRound(const SimulationStatistics& round_stats) {
...
void EvaluationStatistics::endEvaluation() {
    ...
    std::chrono::duration<double> elapsed = eval_clock_end - this->eval_clock_start;
...
void EvaluationStatistics::printEvaluationResults(std::ostream& os) const {
    ...
    if (!this->step_counts_.empty()) {
        ...
        for (int i = 0; i < this->step_counts_.size(); i++) {
            os << right << setw(roundCol.length()) << i << sep << right << setw(stepCol.length())
               << this->step_counts_[i] << sep << right << setw(discRewCol.length()) << fixed
               << this->total_disc_rewards_[i] << sep << right << setw(undiscRewCol.length()) << fixed
...
Evaluator::Evaluator(std::unique_ptr<Planner>&& planner) : planner_(std::move(planner)), round_(0) {
