...
EvaluationStatistics::~EvaluationStatistics() {
    // for (int i = 0; i < round_results_.size(); i++) {
    //     delete round_results_[i];
    // }
}
...
void EvaluationStatistics::summarizeRound(SimulationStatistics* round_stats) {
    ...
    *out_ << fixed << setprecision(2) << "Round(" << right << setw(4) << round_count_ << "): "
          << "reward(" << right << setw(nwidth) << round_stats->total_discounted_reward_ << " / " << right
          << setw(nwidth) << round_stats->total_undiscounted_reward_ << ") "
          << "time(" << right << setw(nwidth) << round_stats->total_round_time << "ms) - "
          << "avg total reward: " << right << setw(nwidth) << avg_disc_rew << " (+-" << stderr_disc_rew << ") / "
          << right << setw(nwidth) << avg_undisc_rew << " (+-" << stderr_undisc_rew << ")" << endl;
    round_count_++;
}
...
void EvaluationStatistics::endEvaluation() {
    ...
    elapsed = eval_clock_end - this->eval_clock_start;
...
void EvaluationStatistics::printRewardStatistics(std::ostream& out) const {
    ...
    out << "Average total discounted reward (stderr) = " << std::to_string(avg_disc_rew) << " ("
        << std::to_string(stderr_disc_rew) << ")" << std::endl;
    // print average and std of undiscounted rewards
    out << "Average total undiscounted reward (stderr) = " << std::to_string(avg_undisc_rew) << " ("
        << std::to_string(stderr_undisc_rew) << ")" << std::endl;
}

void EvaluationStatistics::averageRoundReward(double& discountedAvg, double& undiscountedAvg) const {
...
void EvaluationStatistics::stderrRoundReward(double& discountedStderr, double& undiscountedStderr) const {
    double discAvg = 0.0;
    ...
    if (n <= 1) {
        ...
        // return values
        // divide by n since the results are Monte-Carlo samples (see P. Hennig Probabilistic ML Lecture 4 Slide 10)
        discountedStderr = std::sqrt(sampleVarDis / n);
        undiscountedStderr = std::sqrt(sampleVarUndis / n);
    }
}

void EvaluationStatistics::writeEvalutionResultsToFile(const std::string& filename) const {
...
void EvaluationStatistics::printEvaluationResults(std::ostream& os) const {
    ...
    if (this->step_counts_.size() > 0) {
        ...
        // create header
        stringstream ss;
        ss << roundCol << sep << stepCol << sep << discRewCol << sep << undiscRewCol;
        ...
        for (int i = 0; i < this->step_counts_.size(); i++) {
            os << right << setw(roundCol.length()) << i << sep
               << right << setw(stepCol.length()) << this->step_counts_[i] << sep
               << right << setw(discRewCol.length()) << fixed << this->total_disc_rewards_[i] << sep
               << right << setw(undiscRewCol.length()) << fixed << this->total_undisc_rewards_[i] << endl;
        }
    }
}
...
void Evaluator::saveEvaluationResults() const {
    ...
    ssfn << "Eval_" << this->eval_stats_->round_count_ << "_" << sstime.str() << ".txt";
