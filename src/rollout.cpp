...
IpftValue BeliefRolloutPolicy::rollout(Belief *belief, int depth) const {
  ...
  for (int d = depth; d >= 0; d--) {
    ...
    //* update value
    value += (locReward *
              std::pow(Globals::config.discount_gamma, rolloutIteration));
    rolloutIteration++;
...
Action RandomActionChooser::chooseAction(const Belief *belief) const {
  ...
  int randomActionIndex =
      this->rand_->nextUniformInt(belief->model_->numActions());
...
void DeterministicActionChooser::reset() { round_ = 0; }

/* ----------------- DeterministicSingleActionChooser class ----------------- */

Action
DeterministicSingleActionChooser::chooseAction(const Belief *belief) const {
