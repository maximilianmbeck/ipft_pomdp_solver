...
bool NoReinvigoration::particleReinvigorationNeeded(
    const std::vector<State *> &particleSet, const Action &act,
    const Observation &obs) const {
  return false;
}

std::vector<State *>
NoReinvigoration::reinvigorate(const std::vector<State *> &particleSet,
                               const Action &act,
                               const Observation &obs) const {
  return particleSet;
}
...
bool SimpleParticleReinvigorator::particleReinvigorationNeeded(
    const std::vector<State *> &particleSet, const Action &act,
    const Observation &obs) const {
  return this->allParticlesEqual(particleSet);
}

std::vector<State *> SimpleParticleReinvigorator::reinvigorate(
    const std::vector<State *> &particleSet, const Action &act,
    const Observation &obs) const {
  // all particles are equal
  CHECK(this->allParticlesEqual(particleSet))
      << "Reinvigorate particle set, but reinvigoration criterion not met.";

  State *s = particleSet[0];

  std::vector<State *> reinvigoratedStates =
      this->model_->similarStates(*s, particleSet.size());
  return reinvigoratedStates;
}

bool SimpleParticleReinvigorator::allParticlesEqual(
...
bool ObsAdaptiveReinvigorator::particleReinvigorationNeeded(
    const std::vector<State *> &particleSet, const Action &act,
    const Observation &obs) const {
  ...
  auto stateMaxWeight = std::max_element(particleSet.begin(), particleSet.end(),
                                         State::weightCompare);
