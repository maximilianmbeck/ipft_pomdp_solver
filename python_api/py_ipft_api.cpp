...
// particle np array: one row corresponds to a state, columns correspond to the
// dimensions
std::vector<State *>
...
PyParticleBelief::createPyParticleBelief(const ParticleBelief &pb) {
  ...
  if (!pb.weighted_posterior_particles_.empty())
    particleSet2NpArrays(pb.weighted_posterior_particles_, pyPb.post_particles_,
                         pyPb.post_weights_);
...
public:
  ...
  std::shared_ptr<PyVNode> testTreeConv() {
    ...
    ParticleBelief *b = new ParticleBelief(states, false, this->model_,
                                           this->rand_, new NoReinvigoration());
  ...
  bool testNpParticleArrayConv(const py::array_t<double> &particleArr) {
    ...
    ParticleBelief *bel = new ParticleBelief(
        particleSet, false, this->model_, this->rand_, new NoReinvigoration());
