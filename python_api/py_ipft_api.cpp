...
// void particleSet2NpArrays(const std::vector<State *> &particleSet,
...
// // particle np array: one row corresponds to a state, columns correspond to
// the
// // dimensions
// std::vector<State *>
// npArray2ParticleSet(const py::array_t<double> &unweightedParticleArray,
//                     const POMDP *model) {
//   // check array dimensions
//   if (unweightedParticleArray.ndim() != 2) {
//     throw std::invalid_argument(
//         std::string("Number of dimension must be 2, but is ") +
...
//     throw std::invalid_argument(
//         std::string("Numpy array state dimension ") +
...
//   int numParticles = unwParticleArrData.shape(0); // number of rows of the
//   array int numDims = unwParticleArrData.shape(1); // number of columns of
//   the array
...
// std::shared_ptr<PyVNode>
// PyVNode::convertVNode(const VNode *vnode,
//                       const std::shared_ptr<PyQNode> &qparent) {
...
// std::shared_ptr<PyQNode>
// PyQNode::convertQNode(const QNode *qnode,
//                       const std::shared_ptr<PyVNode> &vparent) {
...
// PyParticleBelief
// PyParticleBelief::createPyParticleBelief(const ParticleBelief &pb) {
...
//     particleSet2NpArrays(pb.weighted_posterior_particles_,
//     pyPb.post_particles_,
//                          pyPb.post_weights_);
...
//   if (i >= this->numSteps()) {
//     throw std::invalid_argument(std::string("Maximum step index is ") +
//                                 std::to_string(numSteps() - 1));
//   }
//   return this->search_trees_.at(i); // access with bounds checking
// }

// int PyPlanner::numSteps() const { return this->step_; }
...
//   this->ipft_ =
//       new Ipft(this->model_, this->initialBelief_, this->rand_,
//                new BeliefInformationPolicy(this->model_, this->rand_));
// }

// void PyIpft::reset() {
//   ParticleBelief *bel =
//       static_cast<ParticleBelief *>(this->initialBelief_->clone());
...
//   py::tuple ret = py::make_tuple(std::move(act), totalValue,
//   std::move(value));
...
//   PyParticleBelief pybel = PyParticleBelief::createPyParticleBelief(
//       *static_cast<ParticleBelief *>(bel));
//   delete bel;
...
//         4.253895, 3.168835, 3.300457,  1.848377, -3.348563,
//         -0.461945, 2.089914, 5.221298,
//         0.026519, 4.701948,  1.938396, 3.295580,  -1.122583, 3.541429,
...
//     // create C++ tree
//     // create root
//     std::vector<double> numbers = {4.253895, 3.168835};
//     std::vector<State *> states =
//         debug::doubleVec2StateVec(numbers, this->model_);
//     ParticleBelief *b = new ParticleBelief(states, false, this->model_,
//                                            this->rand_, new
//                                            NoReinvigoration());
...
//       QNode *qnode =
//           new QNode(this->model_, root, a, 0); // same level as parent vnode
//       qnode->setCount(0);
...
//         npArray2ParticleSet(particleArr, this->model_);
//     ParticleBelief *bel = new ParticleBelief(
//         particleSet, false, this->model_, this->rand_, new
//         NoReinvigoration());
...
//   py::class_<PyVNode, std::shared_ptr<PyVNode> /* <- holder type */> pyVNode(
//       m, "PyVNode", pyNode);
//   pyVNode.def(py::init<>())
...
//   pyIpftCld.def(py::init<const py::array_t<double> &>())
...
//       .def("testParticleBeliefConv",
//       &PyConversionTest::testParticleBeliefConv) .def("testTreeConv",
//       &PyConversionTest::testTreeConv) .def("testNpParticleArrayConv",
//            &PyConversionTest::testNpParticleArrayConv);
