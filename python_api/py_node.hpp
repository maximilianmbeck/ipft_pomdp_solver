...
public:
  ...
  PyParticleBelief belief_;

public:
  static std::shared_ptr<PyVNode> convertTree(const VNode *vroot);
  static std::shared_ptr<PyVNode>
  convertVNode(const VNode *vnode, const std::shared_ptr<PyQNode> &qparent);
};
...
public:
  static std::shared_ptr<PyQNode>
  convertQNode(const QNode *qnode, const std::shared_ptr<PyVNode> &vparent);
};
