...
public:
    ...
    std::unique_ptr<World> initializePOMDPWorld(std::shared_ptr<POMDP> model) override {
        return std::make_unique<POMDPWorld>(model);
    }

    std::unique_ptr<Solver> initializeSolver(std::shared_ptr<POMDP> model, std::unique_ptr<Belief>&& belief) override {

        return std::make_unique<Ipft>(
            this->rand_, model, std::move(belief), std::make_unique<BeliefInformationPolicy>(model, this->rand_));
    }
};
