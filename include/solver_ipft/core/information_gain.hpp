...
public:
    ...
    DiscountedInformationGain& operator=(const DiscountedInformationGain&) = delete;
    DiscountedInformationGain& operator=(DiscountedInformationGain&&) = delete;

    virtual double computeDiscInfGain(double discFactor, const Belief* bnext, const Belief* b) const = 0;
};
...
public:
    EntropyInfGain() {
        densEstimator = std::make_unique<KernelDensityEstimator>(); // default density estimator
    };
    ...
    double computeDiscInfGain(double discFactor, const Belief* bnext, const Belief* b) const override;
