...
struct Config {
  // search parameters
  int search_depth;
  int time_per_move; // in milliseconds, time available to construct the search
                     // tree
  int max_simulation_count;
  int num_search_particles; // number of particle used for the beliefs during
                            // treesearch
  int num_solver_particles; // number of particles of the solver to track the
                            // real world state
  ...
  double inf_gather_constant_lambda; // weights reward maximization and
                                     // information gathering
  double inf_discount_gamma;         // = 1.0 for undiscounted information gain

  // record statistics
  bool record_statistics;
  bool convergence_eval; // after each episode (call to simulate) time and Q-val
                         // stats are recorded
  ...
  Config()
      : search_depth(20), time_per_move(1000), // default 1000
        ...
        record_statistics(true), convergence_eval(false),
