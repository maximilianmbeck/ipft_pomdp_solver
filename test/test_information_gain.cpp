...
protected:
  ...
  InformationGainTest() {
    std::vector<double> numbers = {3.0643, 3.0643, 3.0643, 3.0643, 3.0643,
                                   3.0643, 3.0643, 3.0643, 3.0643, 3.0643,
                                   3.0643, 3.0643, 3.0643, 3.0643, 3.0643,
                                   3.0643, 3.0643, 3.0643, 3.0643, 3.0643};

    std::vector<State *> states =
        debug::doubleVec2StateVec(numbers, this->model_);
    belErr1StepNext = std::make_unique<ParticleBelief>(
        states, false, this->model_, this->rand_);
  }
};
...
TEST_F(InformationGainTest, kdeSilvermansRuleAllPartEqual) { // NOLINT
  ...
  double kdeAllPEqual =
      kde.computeDensityValue(belief2->particle(0), belief2.get());
  LOG(INFO) << "KDE at a state pos in particle set all particles equal : "
            << kdeAllPEqual;
}
...
TEST_F(InformationGainTest, ComputeDiscInfGainErr1Step) { // NOLINT
  ...
  double entrBelErr1StepNext =
      ifm.computeEntropyEstimate(belErr1StepNext.get());
  double infrew =
      ifm.computeDiscInfGain(1.0, belErr1StepNext.get(), belErr1Step.get());
  LOG(INFO) << "entr val belErr1Step: " << entrBelErr1Step;
  LOG(INFO) << "entr val all particles equal: " << entrBelErr1StepNext;
  LOG(INFO) << "inf reward: "
            << infrew * Globals::config.inf_gather_constant_lambda;
